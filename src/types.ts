...
> {
  _parse(input: ParseInput): ParseReturnType<this["_output"]> {
    ...
    let currentDef = this._def;
    let nextDef = this._def.innerType._def;

    while (nextDef != null) {
      currentDef = nextDef;
      nextDef =
        nextDef.innerType != null
          ? nextDef.innerType._def
          : nextDef.schema != null
          ? nextDef.schema._def
          : null;
    }

    const innerType = currentDef.typeName.toLowerCase().replace("zod", "");
